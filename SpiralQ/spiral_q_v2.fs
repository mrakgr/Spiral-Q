...
let remove_biases_flag = false
let no_event_synchro_flag = false
let stream_num = 32

/// Bias Flag Helper. The cuDNN add_tensor is ridiculously slow in the current iteration.
...
let nullary_stream_caller (x : d4M) (f : CudaStream -> CudaEvent -> d4M -> unit) =
    ...
    | Passed(s',e',m') ->
        let s,e,m as sem = StreamPool.P
        if no_event_synchro_flag = false then s.WaitEvent e'.Event
        x.stream_state <- Unpassed sem
        f s e m
        e.Record s.Stream
...
type LSTMLayer =
    ...
    member l.runLayerNoI (y:d4M) (c:d4M) =
        ...
        let c' = linear_layer_hadmult [|block_input,input_gate;c,forget_gate|]
        let output_gate = linear_layer_matmult [|l.U_o,y;l.P_o,c'|] (Some l.b_o |> bfh) |> sigmoid
        hadmult (l.block_output_a c') output_gate, c'
