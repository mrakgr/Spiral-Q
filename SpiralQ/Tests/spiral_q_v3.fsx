...
and d4MUnion =
    ...
    static member create' (ar : (int * int * int * int)[], is_constant) =
        ...
        |> fun x ->
            x.Whole |> ignore // Just a check to make sure the inner dimensions match up.
            x
    ...
    member t.Whole : (int * int * int * int) * CUdeviceptr * CUdeviceptr = t.GetSliceAlongImage(0,t.elements_size.Length-1)
    ...
    member private t.tod4MUnion(size,p,a) =
        {
        elements_size = [|size|]
        elements_primal = [|p|]
        elements_adjoint = [|a|]
        P = new CudaDeviceVariable<float32>(p,false,SizeT (size_nchw size))
        A = 
            if t.A.IsNone then
                None
            else
                new CudaDeviceVariable<float32>(a,false,SizeT (size_nchw size)) |> Some
        is_dead = t.is_dead
        stream_state = t.stream_state
        }

    /// For accessing individual elements with the .[a] syntax.
    member t.Item
        with get(a: int) = 
            (t.elements_size.[a],t.elements_primal.[a],if t.A.IsSome then t.elements_adjoint.[a] else CUdeviceptr(SizeT 0))
            |> t.tod4MUnion

    member t.GetSlice(rowStart: int option, rowFinish : int option) =
        let l = defaultArg rowStart 0
        let r = defaultArg rowFinish (t.elements_size.Length-1)
        t.GetSliceAlongImage(l,r)
        |> t.tod4MUnion
...
/// Matrix-matrix multiply. Uses the TN form.
/// The number of c (output matrix)'s element sizes are based on the b arguments (which is intended to be the weight.)
    ...
    let c = 
        ...
        | None ->
            let size_ar =
                let _,num_rows = a.rc
                [|
                for (num_cols,_,_,_) in b.elements_size do
                    yield num_cols,num_rows,1,1
                    |]
            ObjectPool.getd4M (false, size_ar)
            |> fun c ->
                binary_forward_stream_caller a b c
                <| fun s e m -> gemm T nT 1.0f a.P' b.P' 0.0f c.P' s.Stream
                c
        | Some c ->
            binary_forward_stream_caller a b c
            <| fun s e m -> gemm T nT 1.0f a.P' b.P' 1.0f c.P' s.Stream
            c
    ...
    if a.A.IsSome then 
        let matmult_backward_left () = 
            ...
            <| fun _ -> 
                ...
                <| fun s e m -> gemm nT T 1.0f b.P' c.A' 1.0f a.A' s.Stream
    ...
    if b.A.IsSome then 
        let matmult_backward_right () = 
            deadness_check c b <| fun _ -> 
                ...
                <| fun s e m -> gemm nT nT 1.0f a.P' c.A' 1.0f b.A' s.Stream
