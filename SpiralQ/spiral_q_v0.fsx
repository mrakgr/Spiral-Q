...
type ObjectPool() =
    ...
    member t.getd4M (is_constant, (nchw : (int*int*int*int)[] as p)) =
        ...
        | Some A ->
            ...
            A.MemsetAsync(0uy,s.Stream) // Zeroes out the adjoints concurrently on the forward pass.
            e.Record s.Stream
            t'.adjoint_occupied.Add e
        | None -> ()
...
type DeviceUnaryTransformModule(op: string, unique_name : string) = 
    ...
    member inline t.A((x_nchw, x: CUdeviceptr, x_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.RunAsync(str.Stream, x,o,n) |> ignore
...
type DeviceBinaryTransformModule(op: string, unique_name) = 
    ...
    member inline t.A((x_nchw, x: CUdeviceptr, x_occ),(y_nchw, y: CUdeviceptr, y_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, x,y,o,n) |> ignore
...
type DeviceTrinaryTransformModule(op: string, unique_name) = 
    ...
    member t.A((x_nchw, x: CUdeviceptr, x_occ), (y_nchw, y: CUdeviceptr, y_occ), (z_nchw, z: CUdeviceptr, z_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, x,y,z,o,n) |> ignore
...
type DeviceUnaryMapSumModule(op: string, unique_name) = 
    ...
    member inline t.A((x_nchw, x: CUdeviceptr, x_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, x,o.DevicePointer,n) |> ignore
        o.[SizeT 0]
...
type DeviceBinaryMapSumModule(op: string, unique_name) = 
    ...
    member inline t.A((x_nchw, x: CUdeviceptr, x_occ),(y_nchw, y: CUdeviceptr, y_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, x,y,o.DevicePointer,n) |> ignore
        o.[SizeT 0]
...
type DeviceUnaryCoefTransformModule(op: string, unique_name) = 
    ...
    member inline t.A(coef_x: float32, (x_nchw, x: CUdeviceptr, x_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, coef_x,x,o,n) |> ignore
...
type DeviceBinaryCoefTransformModule(op: string, unique_name) = 
    ...
    member inline t.A(coef_x: float32, (x_nchw, x: CUdeviceptr, x_occ), coef_y: float32, (y_nchw, y: CUdeviceptr, y_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, coef_x,x,coef_y,y,o,n) |> ignore
...
type DeviceTrinaryCoefTransformModule(op: string, unique_name) = 
    ...
    member inline t.A(coef_x: float32, (x_nchw, x: CUdeviceptr, x_occ), coef_y: float32, (y_nchw, y: CUdeviceptr, y_occ), coef_z: float32, (z_nchw, z: CUdeviceptr, z_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.GridDimensions <- dim3(gridSize)
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream, coef_x,x,coef_y,y,coef_z,z,o,n) |> ignore
...
type DeviceMaxColumnActivationModule() = 
    ...
    member t.A(((n : int,c : int,h,w as x_nchw), x: CUdeviceptr, x_occ), (o_nchw, o: CUdeviceptr, o_occ)) =
        ...
        kernel.BlockDimensions <- dim3(block_size)
        kernel.RunAsync(str.Stream,x,o,m,n) |> ignore
...
let geam transa transb (alpha: float32) ((A_num_images, A_num_channels, A_num_rows, A_num_cols as A_nchw), A:CUdeviceptr, A_occ) beta ((B_num_images, B_num_channels, B_num_rows, B_num_cols as B_nchw), B:CUdeviceptr, B_occ) ((C_num_images, C_num_channels, C_num_rows, C_num_cols as C_nchw), C:CUdeviceptr, C_occ) =
    let inline geam (A_num_rows, A_num_cols) (B_num_rows, B_num_cols) (C_num_rows, C_num_cols) =
        ...
        use A = ptr_to_device_variable A_nchw A
        use B = ptr_to_device_variable B_nchw B
        use C = ptr_to_device_variable C_nchw C

        let s,e = StreamPool.P
        wait_on_event s A_occ
        wait_on_event s B_occ
        wait_on_event s C_occ
...
let gemm transa transb (alpha: float32) ((A_num_images, A_num_channels, A_num_rows, A_num_cols), A:CUdeviceptr, A_occ) ((B_num_images, B_num_channels, B_num_rows, B_num_cols), B:CUdeviceptr, B_occ) beta ((C_num_images, C_num_channels, C_num_rows, C_num_cols), C:CUdeviceptr, C_occ) =
    let inline gemm (A_num_rows, A_num_cols) (B_num_rows, B_num_cols) (C_num_rows, C_num_cols) =
        ...
        wait_on_event str A_occ
        wait_on_event str B_occ
        wait_on_event str C_occ
